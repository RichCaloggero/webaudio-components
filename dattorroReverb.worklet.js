...
return [
	...
	["wetGain", 0.3, 0, 1, "k-rate"],	
	["dryGain", 0.6, 0, 1, "k-rate"]
].map(x => new Object({
	name: x[0],
	defaultValue: x[1],
	minValue: x[2],
...
if (inputs.length === 0 || outputs.length === 0 || outputs[0].length < 2) return true;
...
const 	pd   = ~~parameters.preDelay[0]          ,
		...
		we   = parameters.wetGain[0]             * 0.6               , // lo & ro both mult. by 0.6 anyways
		dr   = parameters.dryGain[0]                 ;
